...
const NavBar = () => {
    ...
    // SearchData.map((searchData) => searchData.cardData.map(cardData => {
    //     if (cardData.title.includes(value) && value !== "") {
    //         // const productData = {
    //         //     img: cardData.img,
    //         //     title: cardData.title,
    //         //     price: cardData.price,
    //         //     text: cardData.text,
    //         //     moreDetails: cardData.moreDetails,
    //         // }
    //         // dispatch({
    //         //     type: "", data: {
    //         //         img: cardData.img,
    ...
    const input = useRef()
    const SearchData = useSelector(state => state.sections)
    const [value, setValue] = useState("x")
    const dispatch = useDispatch()
    const Check = SearchData.map((searchData) => {
        return searchData.cardData.map(cardData => {
            return cardData.title.includes(value) && value !== "" && cardData
        })
    }).flat(Infinity)
    const filterCheck = [...Check].filter(e => e !== false)
    filterCheck.map(e => {
        // console.log(e)
        // dispatch({ type: "", data: e })
    })
            ...
            <Container>
                ...
                <Navbar.Collapse id="navbarScroll">
                    <Nav
                        ...
                        navbarScroll
                    >
                        {routes.map((page, i) => <Link to={page.path} className='nav-link' style={{ color: location === page.path && "red" }} key={i}>{page.name}</Link>)}
                    </Nav>
                ...
                <Link to="/Cart" className='cart-link'><CartCheck className='icon' /><span>{CartProducts.length}</span></Link>
            </Container>
        </Navbar>
    );
}
